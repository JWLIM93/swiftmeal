...
function GetRestaurantDetails(){
    if(document.getElementById("restaurant-info-sub-header")!==null){
        ...
        document.getElementById("restaurant-info-sub-header").innerHTML=sessionStorage.getItem('RestName');
        GetReserveLikes();
        GetReviews();
        GetReservations();
    }
...
function GetReserveLikes(){
    status2 = $.ajax({
        url: 'scripts/owner-operations.php?restid='+sessionStorage.getItem('RestID'),
        data: { action: 'getReserveLikes' },
        type: 'post',
        success: function(output) {
            var json = JSON.parse(output);
            document.getElementById("reservations-count-header").innerHTML=json.ReserveLikes.Count;
            document.getElementById("likes-count-header").innerHTML=json.ReserveLikes.Likes;
            document.getElementById("dislikes-count-header").innerHTML=json.ReserveLikes.DisLikes;
        }
    }).responseText;
}
        ...
        success: function(output) {
            ...
            for(i=0;i<json.Reviews.length;i++){
                if(review.indexOf(json.Reviews[i].RID)===-1){
                    DisplayReviews(json.Reviews[i].Name,json.Reviews[i].Content,json.Reviews[i].Date,json.Reviews[i].Time);
                    review.push(json.Reviews[i].RID);
                }
            }
            document.getElementById("reviews-pre-header").innerHTML=json.Reviews.length;
        }
    }).responseText;
}
        ...
        success: function(output) {
            ...
            for(i=0;i<json.Reservations.length;i++){
                if(reserve.indexOf(json.Reservations[i].BID)===-1){
                    DisplayReservations(json.Reservations[i].Name,json.Reservations[i].Pax,json.Reservations[i].Date,json.Reservations[i].Time,json.Reservations[i].BID);
    ...
    status = $.ajax({
        ...
        success: function(output) {
            ...
            if(document.getElementById("restaurant-count-header")!==null){
                document.getElementById("restaurant-count-header").innerHTML=json.Restaurants.length;
            }
            for(i=0;i<json.Restaurants.length;i++){
                if(ownrest.indexOf(json.Restaurants[i].RestID)===-1){
                    DisplayOwnerRestaurants(json.Restaurants[i].Restname, json.Restaurants[i].Street,json.Restaurants[i].RestID);
...
//    -----------last-------------------
    ...
    if(document.getElementById("reservations-list")!==null){
        document.getElementById("reservations-list").appendChild(listnodeseparator);
